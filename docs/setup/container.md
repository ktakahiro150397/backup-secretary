...
- GitHub CLI (`gh`) をコンテナ内で使えるようにする
- Codex CLI (`codex`) をコンテナ内で使えるようにする
- repo管理しているcustom skillsを既存 `/opt/data/skills` へマージ同期できるようにする
- 設定項目と変更時の注意点を明文化する
...
ローカル派生イメージには GitHub CLI (`gh`) を追加している。Hermes から GitHub 操作を行う場合も、実行範囲はコンテナ内と mount 済みディレクトリに限定される。

確認:

```bash
docker compose --profile cli run --rm cli gh --version
```

認証:

```bash
docker compose --profile cli run --rm cli gh auth login
```

`gh` の認証情報はコンテナ内の `HOME=/opt/data` 配下、つまりホスト側の `runtime/hermes-data` 配下に保存される。secret なので commit しない。

## Codex CLI

ローカル派生イメージには Codex CLI (`codex`) を追加している。公式の推奨インストールは `npm i -g @openai/codex`。Codex CLI はローカル端末で動く coding agent で、選択したディレクトリ内のコードを読んだり編集したり、コマンドを実行できる。
...
```bash
docker compose --profile cli run --rm cli codex --version
```

認証は公式 docs では初回実行時に ChatGPT account または API key で行う。公式 README では ChatGPT plan でのサインインが推奨されている。ChatGPT plan を使う場合は、対話 shell でログインする。
...
API key で使う場合は、`runtime/hermes-data/.env` に `OPENAI_API_KEY` を置く。リポジトリ直下 `.env` には置かない。この Compose 構成では `runtime/hermes-data/.env` をコンテナ環境変数として自動注入しないため、CLI から API key を使う場合は shell で読み込む。
...
`codex` の認証情報や設定はコンテナ内の `HOME=/opt/data` 配下、つまりホスト側の `runtime/hermes-data` 配下に保存される。secret なので commit しない。
...
Repo管理しているcustom skillsをHermes runtimeへ同期する場合:

```bash
docker compose --profile skills-sync run --rm --build skills-sync
```

この同期は `./skills` を `/managed-skills:ro` としてmountし、`rsync -a /managed-skills/ /opt/data/skills/` で既存skillsへマージする。`/opt/data/skills` 全体をbind mountで上書きしない。
