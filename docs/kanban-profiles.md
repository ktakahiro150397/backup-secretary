...
- **Reviewer**：github / file / terminal で PR レビュー、セキュリティチェック、lint。Discord 非接続。
...
| `.gitignore` | `profiles/coordinator/researcher/technical/reviewer/config.yaml` をトラッキング許可 |
...
`coordinator`, `researcher`, `technical`, `reviewer` の 4 つが表示されるはず。
...
| `researcher` | `OPENCODE_GO_API_KEY` | coordinator と同じキーを流用可 |
| `technical` | `OPENCODE_GO_API_KEY` | coordinator と同じキーを流用可 |
| `reviewer` | `OPENCODE_GO_API_KEY` | coordinator と同じキーを流用可 |

`.env` やコンテナの `environment:` に追加しておく。

## 次のステップ

1. **動作確認**：Discord で適当な調査依頼を投げ、Coordinator が `kanban_create` でタスク化し、Dispatcher が researcher/technical/reviewer を起動することを確認する。
...
| **調査・文書作成・分析** | `kanban_create` で researcher へ | 「この URL を調べて」「レポート書いて」 |
| **コード書き・システム作業・バグ修正** | `kanban_create` で technical へ | 「その関数を修正して」「デプロイして」 |
| **PR レビュー・セキュリティチェックセlint** | `kanban_create` で reviewer へ | 「この PR レビューして」「コードの問題ない？」 |
...
- Worker プロファイル（researcher/technical/reviewer）は Discord に繋がないため、`send_message` 等は使わない。結果は Kanban → Coordinator がまとめて Discord に通知する形にする。
