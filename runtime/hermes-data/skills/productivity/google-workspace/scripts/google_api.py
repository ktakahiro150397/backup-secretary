...
Usage:
  ...
  python google_api.py tasks lists [--max 10]
  python google_api.py tasks list [--tasklist @default] [--max 20]
  python google_api.py tasks create --title "Task" [--notes "..."] [--due YYYY-MM-DD]
  python google_api.py tasks complete TASK_ID [--tasklist @default]
  python google_api.py tasks delete TASK_ID [--tasklist @default]
...
def _tasks_due_datetime(value: str) -> str:
    """Normalize Google Tasks due input to an RFC3339 timestamp.

    Google Tasks stores due dates as timestamps, but only the date portion is
    meaningful in the product UI. Accept YYYY-MM-DD for convenient CLI use.
    """
    if not value:
        return ""
    if "T" not in value:
...
def _tasks_completed_datetime() -> str:
    return datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")


def _simplify_task(task: dict) -> dict:
    return {
        "id": task.get("id", ""),
...
def _simplify_tasklist(tasklist: dict) -> dict:
    return {
        "id": tasklist.get("id", ""),
...
def tasks_lists(args):
    if _gws_binary():
        ...
        tasklists = [_simplify_tasklist(item) for item in result.get("items", [])]
...
def tasks_list(args):
    ...
    if _gws_binary():
        ...
        tasks = [_simplify_task(item) for item in result.get("items", [])]
...
def tasks_create(args):
    ...
    if args.due:
        body["due"] = _tasks_due_datetime(args.due)

    if _gws_binary():
        result = _run_gws(["tasks", "tasks", "insert"], params={"tasklist": args.tasklist}, body=body)
    else:
        service = build_service("tasks", "v1")
        result = service.tasks().insert(tasklist=args.tasklist, body=body).execute()

    print(json.dumps({
        **_simplify_task(result),
        "status": "created",
    }, indent=2, ensure_ascii=False))


def tasks_complete(args):
    body = {"status": "completed", "completed": _tasks_completed_datetime()}
    if _gws_binary():
        result = _run_gws(
            ["tasks", "tasks", "patch"],
            params={"tasklist": args.tasklist, "task": args.task_id},
            body=body,
        )
    else:
        service = build_service("tasks", "v1")
        result = service.tasks().patch(tasklist=args.tasklist, task=args.task_id, body=body).execute()
...
def tasks_delete(args):
    ...
    print(json.dumps({"status": "deleted", "taskId": args.task_id}))
...
def main():
    ...
    p = tasks_sub.add_parser("list", help="List tasks in a task list")
    ...
    p.add_argument("--due", default="", help="Due date/time, e.g. 2026-04-30 or RFC3339 datetime")
    p.add_argument("--tasklist", default="@default")
    p.set_defaults(func=tasks_create)

    p = tasks_sub.add_parser("complete", help="Mark a task completed")
