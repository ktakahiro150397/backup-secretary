...
### Read-only fallback when `xurl` is unavailable

Use this when the user only wants to understand or capture an X/Twitter URL and `xurl` is not installed/authenticated, `web_extract` cannot fetch X, or the browser is unavailable.

Workflow:

1. Extract the post ID and handle from the URL when possible: `https://x.com/<handle>/status/<id>`.
2. Try public read-only metadata mirrors before giving up:
   - `https://api.fxtwitter.com/<handle>/status/<id>`
   - `https://api.vxtwitter.com/<handle>/status/<id>`
   - `https://publish.twitter.com/oembed?url=<encoded_x_url>`
3. Prefer structured JSON fields (`text`, `author`, `created_at`, `article`, `card`, `media`, `likes/retweets/replies/views`) over scraped HTML.
4. For X Articles or link-card-only tweets, inspect `article.title`, `article.preview_text`, card metadata, and any expanded URL. The tweet text itself may be empty or just a `t.co` / `x.com/i/article/...` link.
5. If mirrors disagree on counts, treat metrics as approximate cache snapshots and say so.
6. Use this fallback only for read-only summarization/capture. Do not use it for posting, liking, following, DMs, or authenticated/private content.
7. Preserve source URLs in summaries or GitHub issues, but do not dump raw JSON unless needed for debugging.

This fallback is especially useful for turning shared X posts into GitHub issues: retrieve enough context, summarize the idea, then follow the `github-issues` skill's external-source issue workflow.
