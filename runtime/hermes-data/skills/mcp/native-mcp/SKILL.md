...
1. **Bind-mount the app into the Hermes container and launch it locally** — fastest PoC.
...
2. **Expose the app's MCP server over HTTP/Streamable HTTP on an internal Docker network** — best production shape when Hermes supports HTTP MCP.
...
3. **Use `docker exec -i <container> ...` as the stdio command** — convenient but risky.
